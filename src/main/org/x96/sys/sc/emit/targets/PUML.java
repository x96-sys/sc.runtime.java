...
public class PUML extends Emitter {

    @Override
    public String visit(Biome biome, String indent) {
        StringBuilder sb = new StringBuilder();
        String folkloreContent = biome.chronicle.accept(this, indent);

        // If there are no organisms and we have folklore (constants), just output the folklore
        if (biome.organisms.isEmpty() && !folkloreContent.trim().isEmpty()) {
            sb.append(folkloreContent);
            return sb.toString();
        }

        // Otherwise, generate full PUML structure
        sb.append(String.format("%s@startuml%n", indent));
        sb.append(String.format("%s%s %s {%n", indent, "package", new String(biome.bioName.raw())));
        String childIndent = indent + I;
        sb.append(folkloreContent);
        for (Organelle o : biome.organisms.values()) {
            sb.append(o.accept(this, childIndent));
            sb.append("\n");
        }
        // Add extra blank line if folklore is NOT empty (EcoSysTest context)
        if (!folkloreContent.trim().isEmpty()) {
            sb.append("\n}");
        } else {
            sb.append("}");
        }
    ...
    public String visit(Bee bee, String indent) {
        StringBuilder sb = new StringBuilder();
        sb.append(String.format("%s%s %s {%n", indent, "struct", new String(bee.id().raw())));
        String childIndent = indent + I;
    ...
    public String visit(Rune rune, String indent) {
        StringBuilder sb = new StringBuilder();
        sb.append(String.format("%senum %s {%n", indent, rune.id().accept(this, "")));

        String elementIndent = indent + I;
        for (Direction direction : rune.directions()) {
            sb.append(String.format("%s%s%n", elementIndent, new String(direction.id().raw())));
        }
        for (Pulse pulse : rune.pulses()) {
            sb.append(pulse.accept(this, elementIndent));
        }
    ...
    public String visit(Hippocampus hippocampus, String indent) {
        ...
        String signal = hippocampus.signal().map(s -> s.accept(this, "")).orElse("");

        // PUML uses !define for constants
        return String.format("%s!define %s %s", indent, id, signal);
    }
