...
public class LLVM extends Emitter {
    public char SS = (char) 0x22; // stringSentinel
    ...
    public String visit(Rune rune, String indent) {
        String totem = new String(rune.id().raw());
        String childIndent = "\n" + indent + "    ";
        ...
        return String.format("%s:totem %s%s%s;%n", indent, totem, insignias, pulses);
    }
    ...
    public String visit(Swarm swarm, String indent) {
        return String.format("%s:logos %s;%n", indent, new String(swarm.id().raw()));
    }
    ...
    public String visit(Dendrite dendrite, String indent) {
        return String.format(
                "%s:gene %s %s;",
                indent,
                dendrite.neurotransmitter().aminoAcidName(),
                dendrite.neurotransmitter().accept(this, ""));
    }
    ...
    public String visit(Id id, String indent) {
        return String.format(":%s", new String(id.raw()));
    }
    ...
    public String visit(Text text, String indent) {
        return String.format(
                "constant [%d x i8] c%s%s%s%s",
                text.raw().length + 1, SS, new String(text.raw()), "\\00", SS);
    }
